...
async function main() {
    ...
    const unlockOp = await bonusWalletLib.unlockWalletOp(wallet, provider, '0x', 100, 1000, 70000, 50000, 60000);
    console.log('callData: ', unlockOp.callData);
    const userOpHash = await unlockOp.getUserOpHashFromContract(
    ...
    unlockOp.signature = encodeSignature(SignatureMode.owner, sigs, 0, 0);
    console.log('lockOp signature: ', unlockOp.signature);
    ...
    const validation = await bundler.simulateHandleOp(unlockOp);
    console.log('validation: ', validation)
    if (validation.status !== 0) {
        throw new Error(`error code:${validation.status}`);
    }

    const bundlerEvent = bundler.sendUserOperation(unlockOp);
